...
- 后端对接策略：混合模式 —— Flow Backend 负责登录与 Todo/Collections/Sync；Notes 直连 Memos。
- 桌面端 IA：Folder 树为主（更像文件管理器）。
- Todo 提醒/重复规则：完整 RRULE（对齐 occurrences 模型）。
...
- 与 `apidocs/*` 对齐：明确 Auth/Todo/Collections/Sync 的资源名、端点、关键字段（`client_updated_at_ms`、`deleted_at`、applied/rejected、server_snapshot、collection_item）。
- 与 Memos API 对齐：明确 Notes/Memos 的端点、ID 规则（`memos/123` 这类含 `/` 的资源名）、离线优先同步与冲突策略。
...
### 对齐验收（必须可核对）
- `PLAN.md` 必须包含“资源-端点-字段对照表”：
  - Flow Backend（Auth/Todo/Collections/Sync）：以 `apidocs/*` 为准。
  - Memos（Notes/Attachments 等）：以 Memos 官方 API 文档为准（并用 `DESIGN.md` 的现有迁移说明补充关键坑位，如 `memos/123` 资源名）。
...
- [x] 1. 收敛关键决策（Decision Log）并给出默认值
...
- [x] 3. Windows 系统集成与窗口规格（托盘/快捷键/无边框/通知/迁移）
...
- [x] 4. 后端对接与“资源-端点-字段”对照表（按 apidocs）

  **What to do**:
  - 新建 `.sisyphus/drafts/plan-sections/04-backend-contracts.md`。
  - 明确桌面端后端策略（用户已选）：混合模式 —— Flow Backend `/api/v1` 负责登录与 Todo/Collections/Sync；Notes 直连 Memos。
  - 写清两套 Base URL 与鉴权/请求头：
    - Flow：`{FLOW_BACKEND_BASE_URL}/api/v1/*` + `Authorization: Bearer <token>` + `X-Request-Id` + device headers
    - Memos：`{MEMOS_BASE_URL}/api/v1/*` + `Authorization: Bearer <token>`（通常来自 Flow 登录返回）
  - 输出一张“资源-端点-字段”对照表（至少覆盖）：
    - Flow/Auth：`/auth/register`、`/auth/login`、`/auth/logout`、`/me`
    - Flow/Todo：lists/items/occurrences（含 `tzid` 规则）+ 相关 sync 资源
    - Flow/Collections：管理接口 + sync 资源 `collection_item`
    - Flow/Sync：`/sync/pull`、`/sync/push`（applied/rejected + server_snapshot）
    - Memos/Memo：list/get/create/update（以及 memoName/ID 规则）
    - Memos/User：`auth/status`、`users/me`（用于 currentUserCreator/过滤策略）
    - Memos/Attachment：上传与下载（与桌面端离线缓存策略的衔接点）
  - 对关键不变量写清楚：
    - Flow：`client_updated_at_ms` 必须单调递增；409 conflict 与 rejected.conflict 的处理。
    - Memos：memoName 可能形如 `memos/123`（含 `/`）；所有作为路由/路径/查询参数的 ID 必须明确 encode/decode 规则。
  ...
  **References**:
  ...
  - `apidocs/api.zh-CN.md`：Auth/Todo/Sync/ErrorResponse 的具体语义。
  - `apidocs/collections.zh-CN.md`：Collections + `collection_item` sync 资源细节。
  - `apidocs/openapi-v1.json`：机器可读对照，避免文档漂移。
  - `DESIGN.md`：Memos API 端点与 URL 规则（尤其 memoName 可能含 `/` 的坑位）。

  **Acceptance Criteria**:
  - [ ] `.sisyphus/drafts/plan-sections/04-backend-contracts.md` 包含对照表与关键字段解释。
  - [ ] 明确写出：`client_updated_at_ms`、`applied/rejected`、`server_snapshot`、以及 `memos/123` 这类 ID 的编码约束。
  ...
  **QA Scenarios**:
  ...
  Scenario: 合同关键字检查
    ...
    Steps:
      ...
      2. rg -n "memos/|memoName|encode|decode|server_url" .sisyphus/drafts/plan-sections/04-backend-contracts.md
    Expected Result: Flow 合同关键字与 Memos ID 规则均出现且被解释。
    Evidence: .sisyphus/evidence/task-04-contracts-keywords.txt
  ```

- [x] 5. 本地 SQLite 数据模型 + 存储目录布局 + 迁移策略

  **What to do**:
  - 新建 `.sisyphus/drafts/plan-sections/05-local-data-model.md`。
  - 定义本地 SQLite 的“最小完整表集”（不做 MVP，覆盖 Memos(Notes)/Todo/Collections/Settings/Sync）：
    - `memos` / `memo_attachments` / （可选）`memo_revisions`
    - `todo_lists` / `todo_items` / `todo_occurrences`
    - `collection_items`
    - `user_settings`（与 sync 资源对齐）
    - `sync_state`（cursor、last_pull_at、last_push_at、last_error）
    - `outbox_mutations`（待 push 队列；与 Flow 的 `/sync/push` 对齐；用于 todo/collections/settings 等）
    - `jobs`（后台任务持久化队列：sync、prefetch、derived rebuild、reminder）
  - 关键补充（混合后端下的“结构引用”必须支持离线）：
    - `collection_items` 的 note_ref 在本地需要同时存：
      - `ref_id`（服务端 ID，例如 `memos/123`，用于最终 push 到 Flow）
      - `ref_local_uuid`（本地 memo uuid，用于“memo 还没同步拿到 server id”时也能先完成整理）
    - 当 Memos 同步拿到 `ref_id` 后，必须触发 backfill，把本地已有的 `ref_local_uuid -> ref_id` 填回，并把相关结构 mutation 写入 outbox。
  ...
  **References**:
  - `DESIGN.md`：Android 端离线优先模型、Memo/Attachment 离线字段、以及 Collections note_ref 双轨（refLocalUuid/refId）思路（用于混合后端下的 backfill 设计）。
  - `apidocs/api.zh-CN.md`：todo/sync/error 字段契约（决定 Flow 侧本地表字段）。
  ...
  **QA Scenarios**:
  ...
  Scenario: 表清单与关键字段覆盖
    Tool: Bash (rg)
    Steps:
      1. rg -n "memos|notes|todo_items|collection_items|outbox|sync_state|jobs" .sisyphus/drafts/plan-sections/05-local-data-model.md
...
- [x] 6. 桌面端完整 UX Flows（可逐条实现）
  ...
  **What to do**:
  ...
  - 覆盖并细化（每条都要有：入口 → 操作 → 状态反馈 → 异常分支）：
    ...
    - Triptych 联动（左栏 Folder 树/Collections；中栏列表；右栏详情/编辑）
    - 拖拽整理（memo/note_ref 拖入 folder、hover 展开、边缘滚动、禁止拖入子孙）
    - 删除/恢复（soft delete、回收站、彻底删除的边界）
    - 分享/导出（ShareCard 图片导出、复制图片/复制文本；可选：若未来接入分享链接再补齐）
    - 冲突提示（Memos：冲突副本；Flow Sync：rejected/server_snapshot 的呈现与操作）
  - 输出一张“快捷键总表”（可配置项也要列出）。
...
- [x] 7. 同步算法与状态机规格（pull/push + applied/rejected）

  **What to do**:
  - 新建 `.sisyphus/drafts/plan-sections/07-sync-spec.md`。
  - 由于用户选择“混合后端”，本章节必须拆成两条同步主干，并写清二者联动点：
    1) **Flow Sync（Todo/Collections/Settings）**（以 `apidocs` 为准）：
    ...
    2) **Memos Sync（Notes/Memos）**（以 `DESIGN.md` 的既有策略为准）：
       - 本地 memo 的 syncStatus 状态机（LOCAL_ONLY/DIRTY/SYNCING/SYNCED/FAILED）
       - 上传：create/update memo + attachments（含失败重试与 lastError）
       - 回拉：轻量刷新与 full sync；保护 dirty 不被覆盖
       - 冲突：按 Android 端策略生成“冲突副本”，避免丢数据
    3) **联动点（结构引用 backfill）**：
       - 当 Memos 为某条本地 memo 分配 `serverId`（形如 `memos/123`）后，必须触发 backfill：把 collection_items.note_ref 的 `ref_local_uuid -> ref_id` 填回，并把结构变更写入 Flow outbox 再 push。
  - 调度触发（两条同步主干都要覆盖）：启动、网络恢复、用户手动同步、睡眠唤醒、周期兜底。
  - 至少 2 张 mermaid（建议 3 张）：
    - Flow：`本地 outbox → /sync/push → /sync/pull → UI`
    - Memos：`本地 memo → create/update → pull → UI`
    -（可选）Backfill：`memo serverId 生成 → 回填 ref_id → push collection_item`。
  ...
  **References**:
  - `apidocs/api.zh-CN.md`：Flow sync pull/push、applied/rejected、server_snapshot、资源列表。
  - `apidocs/collections.zh-CN.md`：collection_item 的 sync 细节（changes.key 为 collection_items）。
  - `DESIGN.md`：MemosSyncWorker 流程、冲突副本策略、attachments/prefetch 思路（作为桌面端等价 job 参考）。

  **Acceptance Criteria**:
  - [ ] `.sisyphus/drafts/plan-sections/07-sync-spec.md` 同时覆盖 Flow Sync 与 Memos Sync（并指出二者边界与联动点）。
  - [ ] 明确写出：applied/rejected/server_snapshot（Flow）与 冲突副本（Memos）。
  - [ ] 含至少 2 个 `mermaid` 图。

  **QA Scenarios**:
  ```
  Scenario: Flow 与 Memos 两条同步主干覆盖
    Tool: Bash (rg)
    Steps:
      1. rg -n "applied|rejected|server_snapshot" .sisyphus/drafts/plan-sections/07-sync-spec.md
      2. rg -n "Memos|memo|冲突副本|LOCAL_ONLY|DIRTY|SYNCING|SYNCED|FAILED" .sisyphus/drafts/plan-sections/07-sync-spec.md
      3. rg -n "backfill|ref_local_uuid|ref_id|memos/" .sisyphus/drafts/plan-sections/07-sync-spec.md
      4. rg -n "```mermaid" .sisyphus/drafts/plan-sections/07-sync-spec.md
    Expected Result: Flow 关键字、Memos 状态机与 backfill 均覆盖；mermaid 段落数 >= 2。
    Evidence: .sisyphus/evidence/task-07-sync-keywords.txt
  ```

- [x] 8. 附件与离线资源策略（缓存/自定义协议/配额/GC）
...
- [x] 9. Electron 安全模型（IPC 白名单 + 参数校验 + 权限边界）
...
- [x] 10. 错误处理与可观测性（request_id、限流、离线、同步失败）
...
- [x] 12. 安装/发布/自动更新（GitHub Actions + 更新 UX）
...
- [x] 15. （可选）在 `DESIGN.md` 顶部增加“桌面端主规格入口”指针
...
- [x] F2. 合同对齐审阅（apidocs 对账） — `unspecified-high`
  - 以 `apidocs/api.zh-CN.md`、`apidocs/collections.zh-CN.md`、`apidocs/to_app_plan.md`（Flow）以及 `DESIGN.md`（Memos 端点与 ID 规则）为基准，核对：
    - endpoint 路径与方法
    - sync resource 名称
    - 错误结构与关键字段语义
    - Memos 侧 memoName（`memos/123`）的 encode/decode 约束是否在 `PLAN.md` 写清
  - 输出：`对齐项 [N/N] | 漂移项 [N]（给出具体段落/关键词） | VERDICT`。
  - Evidence：`.sisyphus/evidence/final-f2-contract-alignment.txt`

- [x] F3. UX/信息架构一致性审阅（Triptych + 键盘优先） — `visual-engineering`
...
- [x] F4. Electron 安全边界审阅（IPC/协议/敏感信息） — `unspecified-high`
...
- `PLAN.md` 明确对接合同：Flow（Auth/Todo/Collections/Sync）与 Memos（Notes/Attachments）两套资源-端点-字段对照表；明确 `client_updated_at_ms` 语义与 memoName（`memos/123`）编码约束。
